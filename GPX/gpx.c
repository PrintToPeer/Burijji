...
static unsigned short read_16(Gpx *gpx)
{
    ...
    u.b[0] = *gpx->buffer.ptr++ & 0xFF;
    u.b[1] = *gpx->buffer.ptr++ & 0xFF;

    return u.s;
}

static short read_16_s(Gpx *gpx)
{
    union {
        short s;
        unsigned char b[2];
...
static void read_extruder_query_response(Gpx *gpx, Sio *sio, unsigned command, char *buffer)
{
    ...
    switch(command) {
        ...
        case 2:
            ...
            sio->response.temperature = read_16_s(gpx);
    ...
    if(length) {
        ...
        do {
            ...
            if(sio->bytes_in) {
                ...
                memset(gpx->buffer.in, 0, BUFFER_MAX);
                // first read
                for(;;) {
                    // read start byte
